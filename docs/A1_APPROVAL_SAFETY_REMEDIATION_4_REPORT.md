...
| content-based `grep -v` secret-scan bypass | removed the heading-content exclusion; the heading was reworded upstream, so the scan passes with no content bypass (`scripts/build-gate.sh`) | secret scan OK on the current tree; a real API-key/token-shaped value is still detected (negative control) |
