#!/bin/bash
# C0D-R3 PreToolUse hook: defer phase. FULL 64-char SHA-256 binding.
# FAILS CLOSED: malformed JSON, missing fields → exit 2.
set -euo pipefail
IN=$(cat)
...
if not sid or not tuid or not tname:
    sys.stderr.write("DEFER_MISSING_FIELD\n"); sys.exit(2)
rec = {"session_id":sid, "tool_use_id":tuid, "tool_name":tname, "input_sha256":inhash}
json.dump(rec, open(3,"w"))
open(4,"w").write(f"{sid}\n{tuid}\n{tname}\n{inhash}\n{tinp}\n")
' <<< "$IN"; PYEXIT=$?
if [ $PYEXIT -ne 0 ]; then
  echo "DEFER_PARSE_FAIL exit=2 pyexit=${PYEXIT}" >> "${C0D_CAP}/life.txt"
