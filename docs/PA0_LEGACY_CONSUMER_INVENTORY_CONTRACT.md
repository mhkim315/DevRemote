...
| --- | --- | --- | --- | --- | --- |
| `internal/term/runtime.go` | `Handlers.Registry`, `WithRegistry`, `RegistryFromContext` | legacy-only consumer | delete | A1 | `managed handlers calling mux.Registry → zero matches` |
| `internal/term/pty.go` | `HandleSessionsAPI`, `HandleSessionCRUD` | legacy-only consumer | delete | A1, A2 | Legacy CRUD routes removed; zero mux.Registry invocations. |
| `internal/term/pty.go` | `HandleWS` | retained PTY primitive | TerminalTransport | A2 | Retained as TerminalTransport WebSocket bridge. |
| `internal/term/telemetry.go` | `HandleSessionsV2`, `mergeLifecycleState`, `buildSimpleSnapshot*` | mixed public composition | Catalog | A3 | `managed status writes from Telemetry/screen/JSONL → deterministic negative test` |
...
| `internal/term/telemetry_service.go` | `Snapshot` / observer polling | legacy-only consumer | delete | A3 | Observer telemetry is completely deleted from managed path. |
| `internal/term/telemetry_service.go` | launch binding/invalidation | mixed public composition | Catalog / Identity | A3 | Retained/migrated to support accurate accepted identity behavior. |
| `internal/term/linkstore.go` | all symbols | legacy-only consumer | delete | A2 | `/api/v2/links route registrations → zero` |
...
| `internal/mux/cmux_delta.go` | snapshot paths | physical deletion target | delete | Phase B | Repository-wide forbidden-reference gate passes. |
| `internal/mux/tmux_adapter.go` | `tmux:` ID, adapter | physical deletion target | delete | Phase B | `tmux/cmux/localpty ID acceptance → zero` |
| `internal/mux/cmux_adapter.go` | `cmux:` ID, adapter | physical deletion target | delete | Phase B | `tmux/cmux/localpty ID acceptance → zero` |
