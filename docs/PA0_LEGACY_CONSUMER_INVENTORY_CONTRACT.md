...
| `internal/term/lifecycle_handlers.go` | `HandleStopSession`, `HandleKillSession`, `HandleDeleteSession` | mixed public composition | Runtime / provider | A2 | `rg "Stop|Kill|Delete" companion-daemon/internal/term/managed_claude.go companion-daemon/internal/term/managed_codex.go` -> positive migration verification (production routes explicitly defined) |
