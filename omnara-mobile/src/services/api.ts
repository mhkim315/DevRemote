...
// DevRemote Go daemon (local HTTP server on port 9171).
// For remote: uses WebRTC P2P via WebRTCTransport, not cloud relay.
const API_BASE_URL = process.env.EXPO_PUBLIC_API_BASE_URL || 'http://localhost:9171';
...
class DashboardAPI {
  private async getAuthHeaders() {
    // DevRemote: Go daemon doesn't require auth for MVP.
    // Future: replace with Firebase Auth or device key.
    try {
    ...
    // Mock session for local dev — daemon accepts unauthenticated requests.
