...
func TestS11B_CorrelationPIDStartRules(t *testing.T) {
	...
	cases := []struct {
		...
		{"adapter mismatch", base, "legacy", "codex", "0.144.1", 4242, start, false},
...
func TestS11B_NoPIDBindingIgnoresProcessIdentity(t *testing.T) {
	...
	if got := LaunchCorrelation(b, "legacy", "codex", "0.144.1", 12345, time.Unix(1, 0)); got != contract.CorrelationUnavailable {
