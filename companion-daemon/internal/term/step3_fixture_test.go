...
// Never depends on host legacy state or socket permissions.
...
func TestStep3_HistoryEndpoint_Returns410(t *testing.T) {
	h := newStep3Handlers(t)
	req := httptest.NewRequest("GET", "/api/sessions?history=legacy:test", nil)
	rec := httptest.NewRecorder()
	h.HandleSessionsV2(rec, req)
	if rec.Code != http.StatusGone {
...
func TestStep3_NormalList_Returns200(t *testing.T) {
	...
	// independent of host legacy state or socket permissions.
