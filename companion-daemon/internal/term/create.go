...
func (h *Handlers) createFromProfile(w http.ResponseWriter, r *http.Request, req createSessionRequest) {
	...
	// DS-CX2: route codex profile to the interactive TUI host when enabled.
	// This creates a controlled_pty:* session with PTY + JSONL tailer.
	if req.ProfileID == "codex" && h.CodexTUI != nil {
		cwd := req.CWD
		if cwd == "" {
			cwd = "/"
		}
		...
		id, err := h.CodexTUI.Create(r.Context(), cwd, deviceID, deviceEpoch)
		if err != nil {
