...
func TestHandleDiagnostic_JSONShape(t *testing.T) {
	...
	svc := NewTelemetryService(reg, events, NewNopLinkStore(), NoopNotifier{}, nil, NewApprovalStore(), nil, nil)
...
func TestHandleDiagnostic_SessionFields(t *testing.T) {
	...
	svc := NewTelemetryService(reg, events, NewNopLinkStore(), NoopNotifier{}, detector, approvals, nil, nil)
