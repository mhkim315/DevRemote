...
func TestParseSessionID_CanonicalPositive(t *testing.T) {
	cases := []struct {
		...
		{"ext:devremote", "ext", "devremote"},
		{"ext:surface:1", "ext", "surface:1"}, // local IDs may contain ':'
...
func TestValidate_MalformedAndEmpty(t *testing.T) {
	cases := []struct {
		...
		{"trailing colon (empty local)", "ext:"},
		{"only colon", ":"},
	}
	for _, tc := range cases {
...
func TestValidate_ControlCharacters(t *testing.T) {
	cases := []SessionRef{
		{Adapter: "tm\nux", LocalID: "x"},
		{Adapter: "ext", LocalID: "a\tb"},
		{Adapter: "ext", LocalID: "a\x00b"},
		{Adapter: "\x1b[31m", LocalID: "x"},
		{Adapter: "ext", LocalID: "del\x7f"},
	}
...
func TestCanonical_NonCanonicalForms(t *testing.T) {
	...
	ref := SessionRef{Adapter: "ext", LocalID: "a:b"}
...
func TestValidateAdapterName_Grammar(t *testing.T) {
	valid := []string{"ext", "ext", "controlled_pty", "codex_app_server", "a", "a0", "a-b_c9"}
	for _, name := range valid {
		if err := ValidateAdapterName(name); err != nil {
			t.Errorf("ValidateAdapterName(%q) = %v, want nil", name, err)
		}
	}
	invalid := []string{"", "Ext", "0ext", "-ext", "_ext", "tm ux", "tm:ux", "tmüx", "ext!", "INVALID_UPPER"}
