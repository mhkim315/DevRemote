...
// type/source vocabulary, confidence in range, and low-confidence events pinned
// to EventUnknown. Returns ErrInvalidEvent (wrapped) on violation.
func ValidateEvent(e AgentEvent) error {
	...
	if len(e.Metadata) > MaxMetadataEntries {
		return errInvalid("metadata exceeds bound")
	}
	return nil
}
...
// unknown/blank type or source becomes EventUnknown / a defaulted source, the
// confidence is clamped, and oversized metadata is dropped. It never fabricates a
// specific typed event — an ambiguous input degrades to EventUnknown.
...
func boundMetadata(m map[string]string) map[string]string {
	out := make(map[string]string, MaxMetadataEntries)
...
// BoundEvents caps a batch to max and reports whether truncation occurred, so a
// caller can surface a degraded result instead of silently dropping the tail.
...
// returns StatusUnknown. It never inspects or emits process lifecycle. Advisory
// provenance (heuristic/prompt_hint/unknown) can win ONLY when no stronger
// evidence exists, and never yields a terminal-sounding claim above medium.
...
// approval. It defaults to NO approval: only an unambiguous EventApprovalRequested
// at or above the confidence floor qualifies. Unknown/low-confidence/near-miss
// events never manufacture an approval.
func SafeApprovalGate(e AgentEvent) bool {
	return e.Type == agent.EventApprovalRequested && e.Confidence >= ApprovalConfidenceFloor
}
